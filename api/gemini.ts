 ...
 */
export async function generateContentWithRetry(params: any, retries = 3, delayMs = 1000): Promise<any> {
  let attempt = 0;
  while (true) {
    try {
...
/**
 * Generate the next interactive and progressive interview question based on history
 */
export async function handleGenerateQuestion(role: string, company: string, difficulty: string, history: any[], focusAreas?: string[]) {
  ...
  return result.text || "Could you tell me more about your previous experience?";
}

/**
 * Analyze an interview and provide a comprehensive feedback report in JSON format
 */
export async function handleAnalyzeInterview(role: string, history: any[]) {
...
/**
 * Generate a technical adaptive quiz for a specified category and difficulty
 */
export async function handleGenerateQuiz(category: string, difficulty: string) {
